...
def descente_gradient(X, Y, theta, alpha, nb_iters):
    ...
    for i in range(0, nb_iters):
        ...
        # print("shape dot(X,theta)",np.dot(X,theta.T).shape)
        theta = theta - alpha * (1/N) * np.sum(( sigmoide(np.dot(X, theta.T)) - Y  ) * X_grad,0)
        # print(theta)
        
        J_history[i] = calcule_cout(X_grad, Y, theta)
